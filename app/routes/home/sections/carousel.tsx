...
const CAROUSEL_ITEMS: CarouselItem[] = [
    {
        ...
        description: "Minuman tradisional yang menyegarkan dengan manfaat kesehatan maksimal",
        rating: 4.8,
        price: "Rp 25.000"
    },
    {
...
export default function CarouselSection() {
    ...
    const goTo = useCallback((index: number) => {
        setCurrentIndex((prev) => {
            const next = ((index % CAROUSEL_ITEMS.length) + CAROUSEL_ITEMS.length) % CAROUSEL_ITEMS.length;
            return next;
        });
        setAutoPlay(false);
    }, []);
    ...
    return (
        ...
        >
            <div className="relative overflow-hidden rounded-2xl shadow-2xl">
                ...
                >
                    {/* Slides container with smooth fade/slide */}
                    <div className="absolute inset-0 flex items-stretch">
                        {CAROUSEL_ITEMS.map((item, idx) => {
                            ...
                            return (
                                <figure
                                    key={item.id}
                                    className={`w-full h-full shrink-0 transition-transform duration-600 ease-in-out ${isActive ? "opacity-100 translate-x-0" : offset < 0 ? "-translate-x-full opacity-0" : "translate-x-full opacity-0"}`}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 bg-gradient-to-t from-black/80 to-transparent text-white">
                        <h2 className="text-2xl md:text-3xl font-extrabold mb-1">{currentItem.title}</h2>
                        <p className="text-gray-200 text-sm md:text-base mb-3">{currentItem.description}</p>
                                ...
                                <div className="flex items-center gap-1" aria-hidden>
                                    {[...Array(5)].map((_, i) => (
                                        <Star
                                            key={i}
                                            size={16}
                                            className={i < Math.round(currentItem.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-400"}
                                        />
                                    ))}
                                </div>
                                <span className="text-yellow-400 font-semibold">{currentItem.rating.toFixed(1)}</span>
                            </div>
                            <span className="text-white text-lg font-bold">{currentItem.price}</span>
                        </div>
                    </div>
            ...
            <div className="flex justify-center gap-2 mt-6" role="tablist" aria-label="Slide selection">
            ...
            <div className="flex items-center justify-center gap-3 mt-6">
                ...
                >
                    {autoPlay ? <><Pause size={16} /> Pause</> : <><Play size={16} /> Resume</>}
                </button>
                <span className="text-sm text-gray-600">Use ← → for navigation, swipe on touch.</span>
            </div>
        </section>
    );
}